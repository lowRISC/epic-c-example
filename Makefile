...
KERNEL_CFLAGS=--target=riscv32 -march=$(ISA) -O3 -mno-relax -DKERNEL

APP_CFLAGS=--target=riscv32 -march=$(ISA) -O3 -mno-relax -fepic
...
kernel: $(CLANG) Makefile kernel.lds init.S kernel.c cm.h htif.c htif.h elf.c elf.h tock.c tock.h util.c util.h
...
cm: $(CLANG) Makefile coremark app.lds htif.c htif.h coremark-port/core_portme.c coremark-port/core_portme.h coremark-port/cvt.c coremark-port/ee_printf.c coremark/core_list_join.c coremark/core_main.c coremark/core_matrix.c coremark/core_state.c coremark/core_util.c
	...
	$(CLANG) --target=riscv32 -fuse-ld=$(LLVM)/bin/ld.lld -nostdlib -static -mno-relax -Wl,--emit-relocs,-T,app.lds -o cm htif_epic.o core_portme.o cvt.o ee_printf.o core_list_join.o core_main.o core_matrix.o core_state.o core_util.o
